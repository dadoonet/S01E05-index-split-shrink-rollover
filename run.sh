...
# Curl Post call Param 1 is the URL, Param 2 is a json file, Param 3 is optional text
curl_post () {
	if [ -z "$3" ] ; then
		echo "Calling POST /$1"
	else 
  ...
  curl -XPOST "$ELASTICSEARCH_URL/$1" -u elastic:$ELASTIC_PASSWORD -H 'Content-Type: application/json' -d"@$2" ; echo
}
...
while true
do
  curl_post "demo-rollover-write/_doc" "elasticsearch-config/foo.json"
  sleep 1
done
